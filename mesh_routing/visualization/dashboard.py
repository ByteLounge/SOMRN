...
class DashboardState:
    ...
    def reset_engines(self):
        ...
        # Non-blocking check for threads
...
def dismiss_intro(n, hidden, mode):
    if n or hidden:
    ...
    try:
        def on_step(t):
            ...
            with state.lock:
                ...
                # Narrator hooks (only for primary)
                if side == 'primary' and state.narrator:
                    ...
                    if engine.metrics.route_breaks > last_breaks:
                        state.narrator.on_link_break(0, 0) # Generic for now as IDs aren't easily piped
...
def update_simulation_and_ui(n, mode, compare, scenario, intro_hidden, proto):
    if not intro_hidden: return dash.no_update
    
    # Detect scenario or protocol change to restart simulation
...
def live_update_components(n, mode, compare):
    with state.lock:
        p = state.primary
        s = state.secondary
        meta = state.scenario_meta
...
def run_dashboard(port=8050, initial_mode='expert', initial_scenario='default', initial_compare=False):
    with state.lock:
        ...
        state.active_scenario = initial_scenario # Triggers simulation restart in callback
